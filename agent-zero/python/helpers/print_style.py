...
# Intentar importar webcolors, si falla usar una implementación alternativa
try:
    import webcolors
    WEBCOLORS_AVAILABLE = True
except ImportError:
    WEBCOLORS_AVAILABLE = False
    # Definir colores básicos para uso sin webcolors
    BASIC_COLORS = {
        "black": (0, 0, 0),
        "white": (255, 255, 255),
        "red": (255, 0, 0),
    ...
    def _get_rgb_color_code(self, color, is_background=False):
        try:
            if color.startswith("#") and len(color) == 7:
                ...
                g = int(color[3:5], 16)
                b = int(color[5:7], 16)
            elif WEBCOLORS_AVAILABLE:
                rgb_color = webcolors.name_to_rgb(color)
                r, g, b = rgb_color.red, rgb_color.green, rgb_color.blue
            else:
                # Usar colores básicos definidos si webcolors no está disponible
                r, g, b = BASIC_COLORS.get(color.lower(), BASIC_COLORS["default"])
